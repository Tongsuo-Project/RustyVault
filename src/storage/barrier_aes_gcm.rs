...
impl SecurityBarrier for AESGCMBarrier {
    ...
    fn unseal(&self, kek: &[u8]) -> Result<(), RvError> {
...
impl AESGCMBarrier {
    ...
    fn encrypt(&self, path: &str, plaintext: &[u8]) -> Result<Vec<u8>, RvError> {
        ...
        let size: usize = EPOCH_SIZE + 1 + iv_len + ciphertext.len() + tag_size;
        ...
        let out_size = EPOCH_SIZE + 1 + iv_len + ciphertext.len() + tag_size;
        if size != out_size {
            out.truncate(out_size);
        }

        //encrypter.set_tag_len(tag_size)?;
        encrypter.get_tag(tag.as_mut_slice())?;

        out[5 + iv_len..5 + iv_len + ciphertext.len()].copy_from_slice(ciphertext.as_slice());
        out[5 + iv_len + ciphertext.len()..out_size].copy_from_slice(tag.as_slice());
    ...
    fn decrypt(&self, path: &str, ciphertext: &[u8]) -> Result<Vec<u8>, RvError> {
        ...
        let raw = &ciphertext[5 + iv_len..ciphertext.len() - tag_size];
        let tag = &ciphertext[ciphertext.len() - tag_size..ciphertext.len()];
        let size = ciphertext.len() - 5 - iv_len - tag_size;
        let mut out = vec![0u8; size];
